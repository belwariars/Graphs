...
void DFS(vector<list<int>> adjlist, int start, int v)
{
	int visited[v+1];
	int i;
	for(i=0;i<=v;i++)
...
int main()
{
   ...
   //     printf("adjlist[%d] ", i);
	...
	int start;
	scanf("%d", &start);
	DFS(adjlist, start, vertices); //No need of starting vertex as we are going to traverse the whole graph(even the disconnected ones also)
